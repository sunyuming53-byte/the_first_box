...
void Arm::Impl::gripper(int position, SpeedRatio /*speed*/, bool blocking) {
    int block_flag = blocking ? 1 : 0;
    if (blocking) {
        std::lock_guard lock(done_mutex_);
        motion_done_ = false;
    ...
    {
        std::lock_guard lock(cmd_mutex_);
        cmd_queue_.push([this, position, block_flag]() {
            int timeout = block_flag ? 30 : 0;
            int ret = rm_set_gripper_position(handle_, position,
                                              block_flag != 0, timeout);
            if (block_flag) {
                last_motion_ok_ = (ret == 0);
                {
                    std::lock_guard lock(done_mutex_);
...
void Arm::gripper(int position, SpeedRatio speed, bool blocking) {
    impl_->gripper(position, speed, blocking);
}
