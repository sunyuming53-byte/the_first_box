...
// DaisPositioningNode — photogate-based reference positioning for D-AIS linear rail.
// Follows CalibNode standalone rclcpp::Node pattern.
// Architecture: subscribes /joint_states + /photogate → records reference →
//   publishes calibrated trajectories to dais_joint_trajectory_controller.
...
public:
        ...
        : Node("dais_positioning_node", options)
    {
        ...
        declare_parameter("photogate_topic", "/photogate/gate0/blocked");
        declare_parameter("debounce_count", 3);
        declare_parameter("homing_velocity", 0.02);
        declare_parameter("jtc_action", "/dais_joint_trajectory_controller/follow_joint_trajectory");

        dais_joint_name_ = get_parameter("dais_joint_name").as_string();
        auto pg_topic = get_parameter("photogate_topic").as_string();
        ...
        photogate_sub_ = create_subscription<std_msgs::msg::Float64>(
            pg_topic, 10,
            [this](const std_msgs::msg::Float64::SharedPtr msg) {
                photogate_callback(msg);
            });

        ref_pos_pub_ = create_publisher<std_msgs::msg::Float64>("/dais/reference_position", 10);

        jtc_client_ = rclcpp_action::create_client<FollowJointTrajectory>(this, jtc_action_name_);
        ...
        homing_srv_ = create_service<std_srvs::srv::Trigger>(
            ...
            [this](const std_srvs::srv::Trigger::Request::SharedPtr,
                ...
                if (std::isnan(reference_position_)) {
                    res->success = false;
                    res->message = "no reference position — trigger photogate first";
                } else {
        ...
        RCLCPP_INFO(get_logger(), "DaisPositioningNode started, joint=%s, gate=%s",
                    dais_joint_name_.c_str(), pg_topic.c_str());
    }
...
private:
    ...
    void photogate_callback(const std_msgs::msg::Float64::SharedPtr msg) {
        bool blocked = msg->data > 0.5;
        std::lock_guard<std::mutex> lock(mutex_);
        ...
        if (blocked != last_blocked_) {
            ...
            if (blocked && !std::isnan(last_dais_position_)) {
                // Rising edge (clear→blocked): record reference position
            ...
            same_count_++;
            if (same_count_ >= debounce_count_) {
    ...
    void diagnose(diagnostic_updater::DiagnosticStatusWrapper& stat) {
        std::lock_guard<std::mutex> lock(mutex_);
        if (std::isnan(reference_position_)) {
            stat.summary(diagnostic_msgs::msg::DiagnosticStatus::WARN, "No reference");
        } else {
        ...
        stat.add("blocked", last_blocked_);
    ...
    void send_homing_trajectory() {
        ...
        point.positions = {reference_position_};
        point.velocities = {homing_velocity_};
        point.time_from_start = rclcpp::Duration::from_seconds(
            std::abs(last_dais_position_ - reference_position_) / homing_velocity_);
        ...
        RCLCPP_INFO(get_logger(), "Sending homing trajectory: ref=%.4f offset=%.4f",
                    reference_position_, current_offset_);
        jtc_client_->async_send_goal(goal, send_goal_options);
    }

    rclcpp::Subscription<sensor_msgs::msg::JointState>::SharedPtr joint_sub_;
    rclcpp::Subscription<std_msgs::msg::Float64>::SharedPtr photogate_sub_;
