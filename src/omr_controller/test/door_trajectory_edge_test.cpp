...
TEST_F(DoorTrajectoryEdgeTest,
       PlanningFailsForAllWaypointsTransitionsToError) {
    ...
    EXPECT_NE(node->getCurrentState(),
...
TEST_F(DoorTrajectoryEdgeTest, ExtremeThetaValuesProduceValidPoses) {
    ...
    for (size_t i = 0; i < waypoints.size(); ++i) {
        ...
        EXPECT_NEAR(norm_sq, 1.0, 1e-6)
            << "Waypoint " << i << " quaternion not normalised";
    }
...
TEST_F(DoorTrajectoryEdgeTest,
       ApproachFromNonHomeSetsUpCollisionObjects) {
    rclcpp::NodeOptions opts;
    opts.append_parameter_override("idle_delay_sec", 0.0);
    opts.append_parameter_override("tick_rate", 100.0);
    auto node = std::make_shared<EdgeTestNode>(opts);
...
TEST_F(DoorTrajectoryEdgeTest, MissingTransformParamDefaultsToZero) {
    ...
    EXPECT_GT(waypoints.size(), 0u)
        << "Waypoints should be generated even with default transform";
...
TEST_F(DoorTrajectoryEdgeTest, ThetaLockedPerPlanCycle) {
    ...
    EXPECT_GE(node->planCallCount(), 6)
        << "Expected at least 6 plan calls (one per waypoint)";
    ...
    if (waypoints.size() >= 2) {
        ...
        double dx = std::abs(waypoints[0].position.x -
                             waypoints[1].position.x);
        double dy = std::abs(waypoints[0].position.y -
                             waypoints[1].position.y);
        double dz = std::abs(waypoints[0].position.z -
                             waypoints[1].position.z);
        double dqw = std::abs(waypoints[0].orientation.w -
                              waypoints[1].orientation.w);
        poses_differ = (dx > 1e-6 || dy > 1e-6 || dz > 1e-6 ||
                        dqw > 1e-6);
    }
    EXPECT_TRUE(poses_differ)
        << "Different (θ, φ) pairs must produce different waypoint poses "
