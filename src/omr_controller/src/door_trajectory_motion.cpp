...
void DoorTrajectoryNode::ensureMoveGroup() {
    ...
    try {
        ...
        RCLCPP_INFO(get_logger(), "MoveGroupInterface initialised for arm_group");
    } catch (const std::exception& e) {
        RCLCPP_ERROR(get_logger(), "Failed to initialise MoveGroupInterface: %s", e.what());
        move_group_.reset();
    }
}
...
void DoorTrajectoryNode::jointStateCallback(const sensor_msgs::msg::JointState::SharedPtr msg) {
...
bool DoorTrajectoryNode::waitForCompletion(const std::vector<double>& target_joints,
                                           double timeout_sec) {
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        ...
        if (current.size() >= 6) {
            ...
            if (all_within_tolerance) {
                RCLCPP_DEBUG(get_logger(), "waitForCompletion: all joints within tolerance");
                return true;
            }
        }
    ...
    RCLCPP_WARN(get_logger(), "waitForCompletion: timeout after %.1f s", timeout_sec);
    return false;
}

// ── Plan + execute + poll ─────────────────────────────────────────────────

bool DoorTrajectoryNode::planAndExecuteToPose(const geometry_msgs::msg::Pose& target) {
    ensureMoveGroup();

    if (!move_group_) {
        RCLCPP_ERROR(get_logger(), "MoveGroupInterface not available — cannot plan");
        return false;
    }
    ...
    if (!static_cast<bool>(plan_result)) {
        RCLCPP_ERROR(get_logger(), "plan failed: %d", static_cast<int>(plan_result.val));
        return false;
    }

    RCLCPP_INFO(get_logger(), "plan succeeded (%.3f s planning time)", plan.planning_time_);

    moveit::core::MoveItErrorCode exec_result = move_group_->execute(plan);

    if (!static_cast<bool>(exec_result)) {
        RCLCPP_ERROR(get_logger(), "execute failed: %d", static_cast<int>(exec_result.val));
        return false;
    }
    ...
    if (points.empty()) {
        RCLCPP_ERROR(get_logger(), "plan trajectory has no points — cannot poll completion");
        return false;
    }

    const auto& target_joints = points.back().positions;
    if (target_joints.size() < 6) {
        RCLCPP_ERROR(get_logger(), "plan trajectory has fewer than 6 joints (%zu)",
                     target_joints.size());
        return false;
    }
...
moveit_msgs::msg::CollisionObject DoorTrajectoryNode::buildDoorPanelMsg(double theta_rad) const {
...
bool DoorTrajectoryNode::planAndExecuteJointHome() {
    ...
    if (!move_group_) {
        RCLCPP_ERROR(get_logger(), "MoveGroupInterface not available — cannot plan home");
        return false;
    }
    ...
    if (!static_cast<bool>(plan_result)) {
        RCLCPP_ERROR(get_logger(), "home plan failed: %d", static_cast<int>(plan_result.val));
        return false;
    }

    RCLCPP_INFO(get_logger(), "home plan succeeded (%.3f s planning time)", plan.planning_time_);

    moveit::core::MoveItErrorCode exec_result = move_group_->execute(plan);

    if (!static_cast<bool>(exec_result)) {
        RCLCPP_ERROR(get_logger(), "home execute failed: %d", static_cast<int>(exec_result.val));
        return false;
    }
