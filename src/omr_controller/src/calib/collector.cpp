...
[[nodiscard]] auto rotation_diversity_ok(std::span<const std::array<double, 6>> poses) -> bool {
    ...
    for (auto i = 1UZ; i < poses.size(); ++i) {  // intentional unsigned
        min_rx = std::min(min_rx, poses[i][3]);
        max_rx = std::max(max_rx, poses[i][3]);
        min_ry = std::min(min_ry, poses[i][4]);
        max_ry = std::max(max_ry, poses[i][4]);
        min_rz = std::min(min_rz, poses[i][5]);
        max_rz = std::max(max_rz, poses[i][5]);
    }

    return (max_rx - min_rx > kRotationThresholdRad) && (max_ry - min_ry > kRotationThresholdRad) &&
            ...
            for (size_t idx = 0; idx < waypoints.size(); ++idx) {
                ...
                if (has_display) {
                    ...
                    auto arm_text = std::format(
                        ...
                        pose.x, pose.y, pose.z, pose.roll * 180.0 / M_PI, pose.pitch * 180.0 / M_PI,
                        pose.yaw * 180.0 / M_PI);
                    cv::putText(display, arm_text, cv::Point(20, 80), cv::FONT_HERSHEY_SIMPLEX, 0.6,
                                cv::Scalar(200, 200, 200), 1);

                    auto status_text =
                        found ? std::format("Auto: Board OK  corners={}", corners.size())
                              : std::format("Auto: Board NOT FOUND");
                    auto status_color = found ? cv::Scalar(0, 255, 0) : cv::Scalar(0, 0, 255);
                    cv::putText(display, status_text, cv::Point(20, 110), cv::FONT_HERSHEY_SIMPLEX,
                                0.6, status_color, 1);
            ...
            while (true) {
                ...
                if (has_display) {
                    ...
                    std::cout << std::format(
                        ...
                        total_, pose.x, pose.y, pose.z, found ? "OK" : "NOT FOUND");
