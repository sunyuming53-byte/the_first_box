...
private:
    ...
    sensor_msgs::msg::Imu::SharedPtr last_imu_;  // 上一帧imu
...
inline ImuProcess::ImuProcess() : b_first_frame_(true), imu_need_init_(true), start_timestamp_(-1) {
    ...
    last_imu_ = std::make_shared<sensor_msgs::msg::Imu>();  // 上一帧imu初始化
}

inline ImuProcess::~ImuProcess() {}

inline void ImuProcess::Reset()  // 重置参数
{
...
// 传入外部参数
inline void ImuProcess::set_param(const V3D &transl, const M3D &rot, const V3D &gyr, const V3D &acc,
                                  const V3D &gyr_bias, const V3D &acc_bias) {
...
inline void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf &kf_state, int &N) {
    ...
    for (const auto &imu : meas.imu)  // 根据所有IMU数据，计算平均值和方差
    {
        ...
        cov_acc =
            cov_acc * (N - 1.0) / N + (cur_acc - mean_acc).cwiseProduct(cur_acc - mean_acc) / N;
    ...
    init_state.grav =
        -mean_acc / mean_acc.norm() * G_m_s2;  // 得平均测量的单位方向向量 * 重力加速度预设值
...
// 反向传播
inline void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf &kf_state,
    ...
    const double &imu_end_time =
        rclcpp::Time(v_imu.back()->header.stamp).seconds();  // 拿到当前帧尾部的imu的时间
    ...
    for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++) {
        ...
        if (rclcpp::Time(head->header.stamp).seconds() < last_lidar_end_time_) {
            dt = rclcpp::Time(tail->header.stamp).seconds() -
                 last_lidar_end_time_;  // 从上次雷达时刻末尾开始传播 计算与此次IMU结尾之间的时间差
        } else {
            dt = rclcpp::Time(tail->header.stamp).seconds() -
                 rclcpp::Time(head->header.stamp).seconds();  // 两个IMU时刻之间的时间间隔
        }
        ...
        double &&offs_t = rclcpp::Time(tail->header.stamp).seconds() -
    ...
    for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--) {
        ...
        // 此时从IMUpose的末尾开始循环，也就是从时间最大处开始，因此只需要判断 点云时间需>IMU
        // head时刻  即可   不需要判断 点云时间<IMU tail
        for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--) {
            dt = it_pcl->curvature / double(1000) - head->offset_time;  // 点到IMU开始时刻的时间间隔

            /*    P_compensate = R_imu_e ^ T * (R_i * P_i + T_ei)    */

            M3D R_i(R_imu * Sophus::SO3::exp(angvel_avr * dt)
            ...
            V3D P_compensate =
                ...
                (imu_state.rot.matrix().transpose() *
                     (R_i * (imu_state.offset_R_L_I.matrix() * P_i + imu_state.offset_T_L_I) +
                      T_ei) -
...
inline double T1, T2;
inline void ImuProcess::Process(const MeasureGroup &meas, esekfom::esekf &kf_state,
