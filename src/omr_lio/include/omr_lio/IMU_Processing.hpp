...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    {
      dt = rclcpp::Time(tail->header.stamp).seconds() - last_lidar_end_time_; //从上次雷达时刻末尾开始传播 计算与此次IMU结尾之间的时间差
    }
    else
    {
      dt = rclcpp::Time(tail->header.stamp).seconds() - rclcpp::Time(head->header.stamp).seconds();     //两个IMU时刻之间的时间间隔
    }
    ...
    double &&offs_t = rclcpp::Time(tail->header.stamp).seconds() - pcl_beg_time;    //后一个IMU时刻距离此次雷达开始的时间间隔
