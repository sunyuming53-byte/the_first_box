...
inline PointCloudXYZI::Ptr normvec(new PointCloudXYZI(100000, 1));
inline PointCloudXYZI::Ptr laserCloudOri(new PointCloudXYZI(100000, 1));
inline PointCloudXYZI::Ptr corr_normvect(new PointCloudXYZI(100000, 1));
inline bool point_selected_surf[100000] = {1};
...
#pragma omp parallel for
    ...
    vectorized_state boxminus(state_ikfom x1, state_ikfom x2) {
        ...
        x_r.block<3, 1>(6, 0) =
            Sophus::SO3(x2.offset_R_L_I.matrix().transpose() * x1.offset_R_L_I.matrix()).log();
    ...
    void update_iterated_dyn_share_modified(double R, PointCloudXYZI::Ptr &feats_down_body,
        ...
        for (int i = -1; i < maximum_iter; i++)  // maximum_iter是卡尔曼滤波的最大迭代次数
        {
            ...
            // 由于H矩阵是稀疏的，只有前12列有非零元素，后12列是零 因此这里采用分块矩阵的形式计算
            // 减少计算量
            ...
            if (!t && i == maximum_iter - 2)  // 如果迭代了3次还没收敛
