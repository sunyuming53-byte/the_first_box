...
void Preprocess::process(const sensor_msgs::msg::PointCloud2::SharedPtr &msg,
    ...
    switch (lidar_type) {
        ...
        default: printf("Error LiDAR Type"); break;
    }
    *pcl_out = pl_surf;
}
...
void Preprocess::rs_handler(const sensor_msgs::msg::PointCloud2::SharedPtr &msg) {
    pl_surf.clear();
    ...
    // Use organized cloud structure to derive ring and time offset
    // msg->height = number of firings, msg->width = number of scan lines
    int height = msg->height;
    int width = msg->width;
    bool organized = (height > 1 && width > 1);

    // scan period in ms (e.g. 10Hz -> 100ms)
    double scan_period_ms = 1000.0 / SCAN_RATE;
    ...
    for (int i = 0; i < plsize; i++) {
        ...
        double dist2 = pt.x * pt.x + pt.y * pt.y + pt.z * pt.z;
        ...
        // Estimate time offset within the scan based on point index
        if (organized) {
            int row = i / width;
            added_pt.curvature = scan_period_ms * row / height;  // ms
        } else {
            added_pt.curvature = scan_period_ms * i / plsize;  // ms
        }
