...
public:
        ...
        : enable_depth_{cfg.enable_depth}
    {
        config_.enable_stream(RS2_STREAM_COLOR,
                              cfg.width, cfg.height, RS2_FORMAT_BGR8, cfg.fps);
        if (enable_depth_) {
            config_.enable_stream(RS2_STREAM_DEPTH,
                                  cfg.width, cfg.height, RS2_FORMAT_Z16, cfg.fps);
        }
        ...
        while (true) {
            ...
            if (enable_depth_) {
                ...
                if (!intrinsics_captured_) {
                    ...
                    depth_intrinsics_.K =
                        (cv::Mat_<double>(3, 3) << i.fx, 0.0, i.ppx,
                                                   0.0, i.fy, i.ppy,
                                                   0.0, 0.0, 1.0);
                    depth_intrinsics_.dist_coeff =
                        (cv::Mat_<double>(1, 5) << i.coeffs[0], i.coeffs[1],
...
CameraStream::CameraStream(const CameraConfig& cfg)
...
auto CameraStream::next() -> std::optional<CameraFrame> {
    return impl_->next();
}
...
CameraStream::operator bool() const {
