...
class Chassis::Impl {
public:
    explicit Impl(const ChassisConfig& config)
        : config_(config), serial_(io_) {}
    ...
    bool connect() {
        ...
        serial_.set_option(boost::asio::serial_port::baud_rate(
                               config_.baud_rate),
                           ec);
        serial_.set_option(
            boost::asio::serial_port::character_size(8), ec);
        serial_.set_option(
        ...
        std::clog << "[m65::Chassis] Serial opened on "
                  << config_.serial_port << " @ " << config_.baud_rate
                  << " baud" << std::endl;

        // Verify baud rate matches hardware
        if (!check_baud()) {
            std::cerr << "[m65::Chassis] Baud check failed on "
    ...
    void disconnect() {
        if (serial_.is_open()) {
            ...
            serial_.close(ec);
            if (ec) {
                std::cerr << "[m65::Chassis] close() failed: "
                          << ec.message() << std::endl;
            }
        }
        std::lock_guard<std::mutex> lock(state_mutex_);
    ...
    ChassisState read_state() {
        if (!serial_.is_open()) {
            std::cerr << "[m65::Chassis] read_state: not connected"
                      << std::endl;
            return ChassisState{};
        }
        ...
        if (!read_motor_data(left_pwm, right_pwm)) {
            std::cerr << "[m65::Chassis] read_motor_data failed"
                      << std::endl;
            return last_state_;
        }
...
private:
    ...
    std::array<uint8_t, kFrameLen> build_frame(
    ...
    void write_frame(const std::array<uint8_t, kFrameLen>& frame) {
        ...
        boost::asio::write(serial_,
                           ...
                           ec);
        if (ec) {
            std::cerr << "[m65::Chassis] write_frame failed: "
                      << ec.message() << std::endl;
        }
    }
    ...
    bool read_frame(std::array<uint8_t, kFrameLen>& frame) {
        ...
        boost::asio::read(serial_,
                          boost::asio::buffer(frame),
                          boost::asio::transfer_exactly(kFrameLen),
                          ec);
        if (ec) {
            std::cerr << "[m65::Chassis] read_frame failed: "
                      << ec.message() << std::endl;
            return false;
        }
        ...
        if (frame[0] != kFrameHead0 || frame[1] != kFrameHead1) {
            std::cerr << "[m65::Chassis] read_frame: bad header ("
                      << std::hex
        ...
        if (!verify_checksum(frame)) {
            std::cerr << "[m65::Chassis] read_frame: checksum mismatch"
                      << std::endl;
            return false;
        }
    ...
    bool check_baud() {
        ...
        if (!read_frame(response)) {
            std::cerr << "[m65::Chassis] check_baud: no response"
                      << std::endl;
            return false;
        }

        if (response[kResultOffset] != kWSuccess) {
            std::cerr << "[m65::Chassis] check_baud: response error "
                      << std::hex
                      << static_cast<int>(response[kResultOffset])
                      << std::dec << std::endl;
            return false;
        }
        ...
        std::clog << "[m65::Chassis] check_baud: reported baud="
                  << baud << ", expected=" << config_.baud_rate
                  << std::endl;
    ...
    bool read_motor_data(int16_t left_pwm, int16_t right_pwm) {
        ...
        if (response[kResultOffset] != kWSuccess) {
            std::cerr << "[m65::Chassis] read_motor_data: command failed "
                      << std::hex
                      << static_cast<int>(response[kResultOffset])
                      << std::dec << std::endl;
            return false;
        ...
        if (response[kCmdOffset] != kMsgIdMotorData) {
            std::cerr << "[m65::Chassis] read_motor_data: wrong cmd "
                      << static_cast<int>(response[kCmdOffset])
                      << std::endl;
            return false;
        }
        ...
        last_state_.version           = static_cast<uint16_t>(
                                          read_int16_be(&response[35]));
    ...
    bool read_motor_status() {
        ...
        if (response[kResultOffset] != kWSuccess) {
            ...
            std::clog << "[m65::Chassis] read_motor_status: command failed "
                      << std::hex
                      << static_cast<int>(response[kResultOffset])
                      << std::dec << std::endl;
            return false;
        ...
        last_state_.battery_temperature =
            static_cast<double>(read_int16_be(&response[5])) * 0.1;
        last_state_.battery_current =
            static_cast<double>(read_int16_be(&response[7])) * 0.01;
