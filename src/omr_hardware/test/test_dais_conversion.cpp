...
inline double pos_rad_to_m(double rad, double lead_m) {
    return rad * lead_m / (2.0 * kPi);
}

/// velocity_rpm → velocity_m_s (prismatic joint)
inline double vel_rpm_to_mps(double rpm, double lead_m) {
    return rpm * lead_m / 60.0;
}

/// velocity_m_s → motor_rad_s (inverse for write path)
inline double vel_mps_to_radps(double mps, double lead_m) {
    return mps * (2.0 * kPi) / lead_m;
}
...
TEST(PositionConversion, ZeroRadToZeroM) {
    EXPECT_DOUBLE_EQ(pos_rad_to_m(0.0, 0.01), 0.0);
}
...
TEST(PositionConversion, HalfRevolution) {
    EXPECT_NEAR(pos_rad_to_m(kPi, 0.01), 0.005, 1e-9);
}
...
TEST(VelocityReadConversion, ZeroRpmToZeroMps) {
    EXPECT_DOUBLE_EQ(vel_rpm_to_mps(0.0, 0.01), 0.0);
}
