...
FrameCapture::FrameCapture(CaptureConfig cfg) : cfg_{std::move(cfg)} {
...
auto FrameCapture::draw_overlay(const camera::CameraFrame& frame,
    ...
    if (!corners.empty()) {
        ...
        cv::line(display, cv::Point(centre.x - 17, centre.y - 17),
                 cv::Point(centre.x + 17, centre.y + 17), cv::Scalar(0, 0, 255), 2);
        cv::line(display, cv::Point(centre.x + 17, centre.y - 17),
                 cv::Point(centre.x - 17, centre.y + 17), cv::Scalar(0, 0, 255), 2);
    }
    ...
    cv::putText(display, count_str, cv::Point(20, 40), cv::FONT_HERSHEY_SIMPLEX, 1.0,
                cv::Scalar(0, 255, 255), 2);

    // ── instructions ──
    cv::putText(display, "[S] Save  [Q] Quit", cv::Point(20, display.rows - 20),
                cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(255, 255, 255), 2);

    return display;
}

void FrameCapture::save(const camera::CameraFrame& frame, int index) const {
    namespace fs = std::filesystem;
...
void FrameCapture::save_with_pose(const camera::CameraFrame& frame, int index,
                                  const ArmPose& pose) const {
    save(frame, index);
    ...
    std::ostringstream ss;
    ss << timestamp_ms() << "," << std::fixed << std::setprecision(6) << pose.tx << "," << pose.ty
       << "," << pose.tz << "," << pose.rx << "," << pose.ry << "," << pose.rz << "\n";
    file << ss.str();
}
