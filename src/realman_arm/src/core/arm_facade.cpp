...
void Arm::moveJ(const JointPosition& target, SpeedRatio speed, bool blocking,
                int trajectory_connect) {
    impl_->moveJ(target, speed, blocking, trajectory_connect);
}
void Arm::moveJ_P(const CartesianPose& target, SpeedRatio speed, bool blocking,
                  int trajectory_connect) {
    impl_->moveJ_P(target, speed, blocking, trajectory_connect);
}
void Arm::moveL(const CartesianPose& target, SpeedRatio speed, bool blocking,
                int trajectory_connect) {
    impl_->moveL(target, speed, blocking, trajectory_connect);
}
void Arm::moveC(const CartesianPose& mid, const CartesianPose& end, SpeedRatio speed, int loop,
                bool blocking) {
...
void Arm::moveJ_CANFD(const JointPosition& t, int m) { Impl::moveJ_CANFD(t, m); }
void Arm::moveP_CANFD(const CartesianPose& t, int m) { Impl::moveP_CANFD(t, m); }
std::vector<std::string> Arm::getWorkFrames() { return Impl::getWorkFrames(); }
void Arm::setWorkFrame(const std::string& n) { Impl::setWorkFrame(n); }
void Arm::enableForceControl(const std::array<double, 6>& p) { Impl::enableForceControl(p); }
void Arm::disableForceControl() { Impl::disableForceControl(); }
