...
auto generate_poses(int count, cv::Size board_size, float square_m,
                    cv::Size image_size, const cv::Mat& K,
                    float min_angle_deg = 15.0f)
  ...
  for (int i = 0; i < count; ++i) {
    ...
    float frac = count > 1 ? static_cast<float>(i) / static_cast<float>(count - 1)
                           : 0.5f;
    ...
    tvecs.push_back(
        (cv::Mat_<double>(3, 1) << rng.uniform(-mx, mx), rng.uniform(-my, my), z));
  }
...
auto build_grid_3d(cv::Size board_size, float square_m)
  ...
  pts.reserve(static_cast<std::size_t>(cols * rows));
...
std::vector<cv::Mat> generate_chessboard_images(int count,
  ...
  auto [rvecs, tvecs] =
      generate_poses(count, board_size, square_m, image_size, K);
...
std::vector<cv::Mat> generate_charuco_images(int count,
  ...
  auto dict_ptr =
      cv::Ptr<cv::aruco::Dictionary>(new cv::aruco::Dictionary(dict));
  auto board = cv::aruco::CharucoBoard::create(squares_xy.width,
                                                squares_xy.height, square_len,
                                                marker_len, dict_ptr);

  int px_per_sq = 200;
  cv::Size board_px(squares_xy.width * px_per_sq,
                    squares_xy.height * px_per_sq);
  ...
  std::vector<cv::Point2f> src_pts = {
      ...
      {static_cast<float>(board_px.width - 1),
       static_cast<float>(board_px.height - 1)},
      {0.0f, static_cast<float>(board_px.height - 1)},
  };

  auto [rvecs, tvecs] =
      generate_poses(count, squares_xy, square_len, image_size, K);
