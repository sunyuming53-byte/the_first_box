...
TEST(PoseProcessorTest, EyeToHandThreePoses) {
    ...
    // Per the header doc: EyeInHand = inv(T_{i+1})*T_i,
    //                    EyeToHand = T_{i+1}*inv(T_i).
    // These should differ for non-commuting transforms.
...
TEST(PoseProcessorTest, InsufficientPoses) {
    ...
    // Error should mention "at least 2" (see pose_proc.cpp line 87).
