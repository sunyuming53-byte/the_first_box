...
#include <cstdio>
#include <cstdlib>
// NOLINTBEGIN(cert-err33-c,modernize-use-std-print)
...
int main() {
    ...
    auto images = rm::calib::test::generate_chessboard_images(kCount, kBoardSize, kSquareM, K, dist,
                                                              kImageSize);
    ...
    for (int i = 0; i < kCount; ++i) {
        ...
        bool found = cv::findChessboardCorners(images[i], kBoardSize, corners);
        if (found) {
            ++detected;
        } else {
...
// NOLINTEND(cert-err33-c,modernize-use-std-print)
